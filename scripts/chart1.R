...
ggplot(data = data_test) +
  geom_line(mapping = aes(x = week,
                          y = percent_change, color = travel_category))
...
plot_1 <- function(df) {
  df <- df %>%
    ...
    summarise(
      r_and_r = round(mean(retail_and_recreation_percent_change_from_baseline,
                           na.rm = TRUE)),
      g_and_p = round(mean(grocery_and_pharmacy_percent_change_from_baseline,
                           na.rm = TRUE)),
      parks = round(mean(parks_percent_change_from_baseline,
                         na.rm = TRUE)),
      transit = round(mean(transit_stations_percent_change_from_baseline,
                           na.rm = TRUE)),
      workplace = round(mean(workplaces_percent_change_from_baseline,
                             na.rm = TRUE)),
      residential = round(mean(residential_percent_change_from_baseline,
                               na.rm = TRUE))
    )
}
