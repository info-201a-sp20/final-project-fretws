...
data_test <- data %>%
  ...
  group_by(date, travel_category) %>%
  summarise(
    mean_percent_change = round(mean(percent_change, na.rm = TRUE))
  ) %>%
  group_by(date)
ggplot(data = data_test) +
  geom_line(mapping = aes(x = date, y = mean_percent_change, color = travel_category))
